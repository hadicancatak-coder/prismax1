...
export default function DisplayPlanner() {
  ...
  const { data: ads = [], isLoading, refetch } = useQuery({
    queryKey: ["ads-display", entityFilter, statusFilter],
    queryFn: async () => {
      let query = supabase.from("ads").select("*").eq("ad_type", "display");
  ...
  const handleSaveAd = async (adData: any) => {
    if (selectedAdForEdit?.id) {
      ...
      const { error } = await supabase.from("ads").insert({ ...adData, ad_type: 'display' });
      if (error) {
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Display Planner</h1>
          <p className="text-muted-foreground">Build and manage Display/GDN ad campaigns</p>
        </div>
        
        <Tabs value="display" className="w-auto">
          <TabsList>
