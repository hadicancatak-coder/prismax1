  ...
  const handleSave = async () => {
    ...
    try {
      const { error } = await supabase.from("ads").insert({
        name: `Snap Ad - ${adData.headline.substring(0, 30)}`,
        ad_type: "snapchat",
        headlines: [{ text: adData.headline }],
        business_name: adData.brandName,
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold">Snapchat Ad</h3>
      ...
      <div>
        ...
        <Input
          ...
          onChange={(e) => setAdData({ ...adData, brandName: e.target.value })}
          placeholder="CFI Group"
        />
      </div>
