...
// Password V5 validation schema (enhanced security - allows underscores)
export const passwordV5Schema = z.string()
  .min(12, "Password must be at least 12 characters")
  .max(128, "Password must not exceed 128 characters")
  .regex(/[A-Z]/, "Password must contain at least one uppercase letter")
  .regex(/[a-z]/, "Password must contain at least one lowercase letter")
  .regex(/[0-9]/, "Password must contain at least one number")
  .regex(/[!@#$%^&*()_\-+={[}\]|:;"'<>,.?/\\]/, "Password must contain a special character (e.g. _ ! @ # $ %)")
  .refine(
    (password) => {
...
// Auth schema (for login/signup) - uses stronger password validation
export const authSchema = z.object({
  email: z.string()
    .email("Invalid email address")
    .regex(/@cfi\.trade$/, "Only @cfi.trade email addresses are allowed"),
  password: passwordV5Schema,
  name: z.string()
    .min(1, "Name is required")
